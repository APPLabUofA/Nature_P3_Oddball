function Preprocessing_Nat_P3(exp)
...
%                 end
            ...
            if isempty(exp.preocularthresh) == 0
                ...
                rejtrial(i_part,3).ids = EEG.trials;
                for i_rej = 1:length(EEG_temp.epoch)
                    if length(EEG_temp.epoch(1,i_rej).eventtype) == 1
                        master_list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    elseif length(exp.events(1,:)) == 2
                        master_list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    else
                        master_list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{2};
                    end
                end
                for i_rej = 1:length(rejtrial(i_part,1).ids)
                    if length(EEG_temp.epoch(1,i_rej).eventtype) == 1
                        list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    elseif length(exp.events(1,:)) == 2
                        list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    else
                        list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{2};
                    end
                end
                rejtrial(i_part,1).rej = list;
            end
            ...
            if isempty(exp.postocularthresh) == 0
                ...
                rejtrial(i_part,4).ids = EEG.trials;
                for i_rej = 1:length(EEG_temp.epoch)
                    if length(EEG_temp.epoch(1,i_rej).eventtype) == 1
                        master_list_2{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    elseif length(exp.events(1,:)) == 2
                        master_list_2{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    else
                        master_list_2{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{2};
                    end
                end
                for i_rej = 1:length(rejtrial(i_part,2).ids)
                    if length(EEG_temp.epoch(1,i_rej).eventtype) == 1
                        list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                    elseif length(exp.events(1,:)) == 2
                        list{i_rej} = EEG_temp.epoch(1,i_rej).eventtype{1};
                ...
                rejtrial(i_part,2).rej = list;
            end
            
            save(['rejected_' exp.participants{i_part}],'rejtrial','master_list','master_list_2');
